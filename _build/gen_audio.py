...
c.append(code(r"""!pip -q install librosa soundfile pandas numpy scikit-learn lightgbm transformers torch
...
c.append(md(r"""## วิธีที่ 1.5 — CLAP zero-shot (ของแรงปี 2026 ไม่ต้องเทรน แนะนำลองก่อน!)

ใช้โมเดล `CLAP` ทายคลาสจาก "ชื่อคลาสเป็นข้อความ" ตรง ๆ ไม่ต้องเทรน มักแม่นกว่า MFCC features เยอะ
(ถ้ามี label เยอะ อยากเทรน -> ใช้ embedding `mispeech/ced-base` แทน Xtr/Xte แล้วต่อวิธีที่ 1/2 — ดู LIBRARIES.md)"""))
c.append(code(r"""from transformers import pipeline
import torch
clf = pipeline("zero-shot-audio-classification", model="laion/clap-htsat-fused",
               device=0 if torch.cuda.is_available() else -1)   # << เปลี่ยนเป็น laion/larger_clap_general ได้
labels = sorted(df[LABEL_COL].astype(str).unique().tolist())     # ชื่อคลาสเป็นข้อความ (CLAP ใช้ตรง ๆ)
ans = []
for i in sample[ID_COL].tolist():
    p = os.path.join(TEST_AUD, str(i)+TEST_EXT)
    ans.append(clf(p, candidate_labels=labels)[0]["label"])
out = sample.copy(); out[ANS_COL] = ans
out.to_csv("submission_clap.csv", index=False)
print("บันทึก submission_clap.csv (zero-shot ไม่ต้องเทรน)"); display(out.head())"""))

c.append(md(r"""## วิธีที่ 1 — ฟีเจอร์ MFCC + AutoGluon (baseline ไม่ต้อง GPU)"""))
